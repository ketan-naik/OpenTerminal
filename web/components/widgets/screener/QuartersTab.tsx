...
export default function QuartersTab({
  ...
  const [viewMode, setViewMode] = useState<"values" | "yoy" | "qoq" | "commonsize">("values");
  ...
  const { periods, incomeStatement, cashFlowStatement } = financials;
  ...
  const formatCell = (row: FinancialStatementRow, periodKey: string, pIdx: number) => {
    ...
    if (viewMode === "yoy" && pIdx >= 4) {
      const prev = row.values[periods[pIdx - 4].key];
      if (prev === null || prev === undefined || prev === 0) return "—";
      const g = ((raw - prev) / Math.abs(prev)) * 100;
      return `${g >= 0 ? "+" : ""}${g.toFixed(1)}%`;
    }

    // Default Currency / Percent formatting
  ...
  const allRows = [...incomeStatement, ...cashFlowStatement];
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 p-3 bg-[var(--panel-2)] border border-[var(--border)] rounded">
        <div>
          <h3 className="font-bold text-[13px] text-[var(--amber)] flex items-center gap-1.5">
            <span>📊</span>
            <span>Quarterly Financial Results ({periods.length} Quarters)</span>
          </h3>
          <p className="text-[11px] text-[var(--text-dim)]">
            Click any financial row to render an interactive trend chart.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-[var(--panel)] p-0.5 rounded border border-[var(--border)] text-[11px] font-mono">
          ...
          <button
            ...
            className={`px-2 py-0.5 rounded font-bold transition-colors ${
              viewMode === "yoy" ? "bg-[var(--amber)] text-black" : "text-[var(--text-dim)] hover:text-white"
            }`}
          >
            YOY GROWTH %
          </button>
          <button
            ...
            className={`px-2 py-0.5 rounded font-bold transition-colors ${
              viewMode === "qoq" ? "bg-[var(--amber)] text-black" : "text-[var(--text-dim)] hover:text-white"
            }`}
          >
            QOQ GROWTH %
          </button>
        ...
        <table className="w-full text-left border-collapse text-[11px] font-mono">
          <thead>
            <tr className="bg-[var(--panel)] border-b border-[var(--border)] text-[var(--text-dim)] uppercase text-[10px]">
              <th className="p-2.5 sticky left-0 bg-[var(--panel)] z-10 min-w-[200px]">Financial Metric</th>
              {periods.map((p) => (
